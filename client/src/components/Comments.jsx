...
export function ReturnComments({ requestId, currentUser }) {
  const [comments, setComments] = useState([]);
  const [body, setBody]         = useState("");
  const [loading, setLoading]   = useState(false);
  const [sending, setSending]   = useState(false);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/returns/${requestId}/comments`);
      setComments(data || []);
    } catch { toastError("Gabim gjatë ngarkimit të komenteve."); }
    finally { setLoading(false); }
  };
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      await api.post(`/returns/${requestId}/comments`, { body: body.trim() });
      setBody("");
      load();
    } catch { toastError("Gabim gjatë dërgimit."); }
    finally { setSending(false); }
  };

  return (
    <div className="flex flex-col gap-3">
      {loading && <p className="text-sm text-slate-400 text-center py-4">Duke ngarkuar...</p>}
      {!loading && comments.length === 0 && (
        <p className="text-sm text-slate-400 text-center py-4">Nuk ka komente ende.</p>
      )}
        ...
        {comments.map(c => {
          ...
          return (
            <div key={c.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
              <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${isMe ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-800"}`}>
                {c.body}
              </div>
              <span className="text-xs text-slate-400 mt-0.5 px-1">
                {c.first_name} {c.last_name}
                <span className={`ml-1 text-xs px-1.5 py-0.5 rounded-full ${ROLE_COLOR[c.role] || "bg-gray-100 text-gray-600"}`}>{c.role}</span>
      ...
      <div className="flex gap-2 mt-2 border-t border-slate-100 pt-3">
        <textarea
          ...
          rows={2} placeholder="Shkruaj koment..."
        ...
        <button onClick={send} disabled={sending || !body.trim()}
          className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-50 self-end">
          {sending ? "..." : "Dërgo"}
        </button>
      </div>
      <p className="text-xs text-slate-400">Ctrl+Enter për dërgim të shpejtë</p>
    </div>
  );
}
