...
export default function Agent() {
  ...
  useEffect(() => {
    ...
    // Nëse lot është zgjedhur (ekzakt me suggestion) — mos kërko sërish
    ...
    <Layout profile={profile}>
      ...
      {gallery.open && (
        <div className="fixed inset-0 z-50 bg-black/90 flex flex-col" onClick={closeGallery}>
          ...
          {/* Image area - full remaining height */}
          <div className="flex-1 relative flex items-center justify-center min-h-0 px-2" onClick={closeGallery}>
            <img
              src={`${API_BASE}${gallery.urls[gallery.idx]}`}
              alt=""
              className="max-w-full max-h-full object-contain rounded-lg select-none"
              onClick={e => e.stopPropagation()}
            />

            {/* Left arrow - overlaid on image area */}
            {gallery.idx > 0 && (
              <button
                onClick={e => { e.stopPropagation(); setGallery(s => ({...s, idx: s.idx-1})); }}
                className="absolute left-2 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center rounded-full bg-black/50 hover:bg-black/70 text-white text-3xl leading-none transition-colors shadow-lg">
                ‹
              </button>
            )}

            {/* Right arrow - overlaid on image area */}
            {gallery.idx < gallery.urls.length-1 && (
              <button
                onClick={e => { e.stopPropagation(); setGallery(s => ({...s, idx: s.idx+1})); }}
                className="absolute right-2 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center rounded-full bg-black/50 hover:bg-black/70 text-white text-3xl leading-none transition-colors shadow-lg">
                ›
              </button>
            )}
          </div>
