// server.js â€” Fin Approvals Enterprise v3
...
const loginLimiter = rateLimit({ windowMs:60_000, max:10, message:{error:"ShumÃ« pÃ«rpjekje. Provo pas 1 minute."} });
const resetLimiter = rateLimit({ windowMs:15*60_000, max:5 });
const apiLimiter   = rateLimit({ windowMs:60_000, max:400 });
app.use(apiLimiter);

/* â”€â”€â”€ API prefix â”€â”€â”€ */
...
/* â”€â”€â”€ IP Whitelist middleware for admin routes â”€â”€â”€ */
...
function pdfFromRequestRows({reqRow,items,approvals,watermark}) {
  return new Promise((resolve,reject)=>{
    const PDFDoc=new PDFDocument({size:"A4",margin:0,info:{Title:"KÃ«rkesÃ« pÃ«r Lejim Financiar",Author:"Fin Approvals"}});
    ...
    setB();PDFDoc.fontSize(14).fillColor("#ffffff").text("KÃ‹RKESÃ‹ PÃ‹R LEJIM FINANCIAR",0,12,{width:PW,align:"center"});
    setR();PDFDoc.fontSize(8.5).fillColor("rgba(255,255,255,0.72)").text(`#${reqRow.id}  Â·  ${fmtD(reqRow.created_at)}  Â·  Gjeneruar automatikisht`,0,31,{width:PW,align:"center"});
    Y=62;
    ...
    PDFDoc.text("TÃ‹ DHÃ‹NAT E AGJENTIT",ML+8,Y+7);
    setB();PDFDoc.fontSize(9).fillColor("#555");
    const agRows=[
      ["Agjenti:",[`${cl(reqRow.agent_first)} ${cl(reqRow.agent_last)}`.trim()]],
      ["PDA:",[cl(reqRow.agent_pda)||"â€”"]],
      ["Divizioni:",[cl(reqRow.division_name)||"â€”"]],
    ];
    ...
    const rx=ML+IW+10;
    setB();PDFDoc.fontSize(7).fillColor("#8b9cb3");
    PDFDoc.text("TÃ‹ DHÃ‹NAT E BLERJES",rx+8,Y+7);
    const buyRows=[
      ["BlerÃ«si:",`${cl(reqRow.buyer_code)} ${cl(reqRow.buyer_name)}`.trim()],
      ["Objekti:",reqRow.site_code?`${cl(reqRow.site_code)} â€” ${cl(reqRow.site_name)}`:cl(reqRow.site_name)||"â€”"],
      ["Nr. faturÃ«s:",cl(reqRow.invoice_ref)||"â€”"],
      ["Arsyeja:",cl(reqRow.reason)||"â€”"],
    ];
    ...
    // SKU(54) Art(150) Base(54) Qty(26) Rabat%(46) PB(52) Lejim%(46) DDV%(36) Final(58) + 8 for padding â‰ˆ 535
    const TH=["SKU","Artikulli","Ã‡mimi bazÃ«","Qty","Rabat %","Ã‡mimi PB","Lejim %","Final â‚¬"];
    const TW=[70,164,54,26,46,52,46,58];  // DDV hequr, hapesira shperndarÃ« te SKU+Art
    ...
    items.forEach((it,idx)=>{
      ...
      else{PDFDoc.fillColor("#aaa");PDFDoc.y=mid;PDFDoc.text("â€”",cx+3,mid,{width:TW[4]-6,align:"right",lineBreak:false});}
      cx+=TW[4];
      setB();PDFDoc.fillColor("#1e3a5f");PDFDoc.y=mid;PDFDoc.text(it.pbPrice!=null?fm4(it.pbPrice):"â€”",cx+3,mid,{width:TW[5]-6,align:"right",lineBreak:false});cx+=TW[5];
    ...
    PDFDoc.text(`â‚¬ ${fm2(total)}`,ML+TSUM-TW[7]+3,Y+5,{width:TW[7]-6,align:"right"});
    Y+=28;
    ...
    const slabel=isApp?"E aprovuar":isRej?"E refuzuar":"NÃ« pritje";
    ...
    setR();PDFDoc.fontSize(8).fillColor("#8b9cb3").text(`Niveli: ${cl(reqRow.required_role)||"â€”"}`,ML+8,Y+42);
    ...
    if(last){
      ...
      PDFDoc.text(`Koment: ${cl(last.comment)||"â€”"}`,ax+8,Y+46,{width:BW-16,ellipsis:true});
    }else{
    ...
    PDFDoc.text("Fin Approvals Â· Dokument konfidencial",ML,FY+10,{width:CW/2});
    PDFDoc.text(`Gjeneruar: ${fmtD(new Date())} Â· Faqe 1/1`,ML+CW/2,FY+10,{width:CW/2,align:"right"});
...
app.post("/auth/login", loginLimiter, async(req,res)=>{
  try {
    ...
    if(!email||!password)return res.status(400).json({error:"Email dhe fjalÃ«kalimi janÃ« tÃ« detyrueshme"});
    const r=await q("SELECT * FROM users WHERE email=$1",[email]);
    if(!r.rowCount)return res.status(401).json({error:"Kredenciale tÃ« gabuara"});
    const u=r.rows[0];
    if(!u.password_hash||!(await compare(password,u.password_hash)))return res.status(401).json({error:"Kredenciale tÃ« gabuara"});
    ...
    if(!known.rowCount) {
      ...
      if(u.email) {
        sendMail({
          to:u.email,
          subject:"[Fin Approvals] Hyrje e re e dyshimtÃ«",
          html:`<p>U identifikua hyrje nga pajisje/IP e re:<br><b>IP:</b> ${ip}<br><b>Browser:</b> ${req.headers["user-agent"]||"i panjohur"}<br><b>Koha:</b> ${new Date().toLocaleString()}</p><p>NÃ«se nuk jeni ju, ndryshoni fjalÃ«kalimin menjÃ«herÃ«.</p>`
        }).catch(()=>{});
      }
    } else {
...
app.post("/auth/2fa/verify", async(req,res)=>{
  try {
    ...
    if(!temp_token||!totp_code)return res.status(400).json({error:"Parametra mungojnÃ«"});
    const stored=await q("SELECT * FROM refresh_tokens WHERE token=$1 AND revoked=FALSE AND expires_at>NOW()",["2fa:"+temp_token]);
    if(!stored.rowCount)return res.status(401).json({error:"Token i pavlefshÃ«m"});
    ...
    if(!u.rowCount)return res.status(401).json({error:"PÃ«rdoruesi nuk u gjet"});
    const user=u.rows[0];
    if(!verifyTotp(totp_code,user.totp_secret))return res.status(401).json({error:"Kodi TOTP Ã«shtÃ« i gabuar"});
...
app.post("/auth/reset-password", async(req,res)=>{
  try {
    ...
    if(!token||!password||password.length<6)return res.status(400).json({error:"Token dhe fjalÃ«kalim i ri (min 6 karaktere) kÃ«rkohen"});
    const r=await q("SELECT * FROM password_reset_tokens WHERE token=$1 AND used=FALSE AND expires_at>NOW()",[token]);
    if(!r.rowCount)return res.status(400).json({error:"Token i pavlefshÃ«m ose i skaduar"});
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ PRICINGBRIDGE PROXY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */

/* â”€â”€ Local article search (sku + barkod + emÃ«r) â”€â”€ */
app.get("/articles/search", requireAuth, async(req,res)=>{
  ...
  const t = term.trim();
  try {
    // KÃ«rko nÃ« bazÃ«n lokale: me SKU, barkod, ose emÃ«r
    let divFilter = "";
    const params = [`%${t}%`, `%${t}%`, `%${t}%`];
    let paramIdx = 3;

    // NÃ«se Ã«shtÃ« agjent, filtro sipas divisioneve
...
app.get("/pb/article", requireAuth, async(req,res)=>{
  ...
  try{
    ...
    return res.status(502).json({error:"PricingBridge nuk disponueshÃ«m",detail:e.message});
  }
});

app.get("/pb/price", requireAuth, async(req,res)=>{
  const{sifraKup,sifraObj,sifraArt,lotBr}=req.query;
  if(!sifraKup||!sifraArt) return res.status(400).json({error:"sifraKup dhe sifraArt tÃ« detyrueshme"});
  try{
    ...
    if(!result) return res.status(404).json({error:"Ã‡mimi nuk u gjet"});
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ META â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ THRESHOLDS (Admin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
app.put("/admin/thresholds", requireAuth, requireRole("admin"), async(req,res)=>{
  ...
  if(!team_lead_max||!division_manager_max)return res.status(400).json({error:"Parametra mungojnÃ«"});
  if(Number(team_lead_max)>=Number(division_manager_max))return res.status(400).json({error:"Team Lead max duhet tÃ« jetÃ« mÃ« i vogÃ«l se Division Manager max"});
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ APPROVER LIST (for delegations) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
app.get("/users/approvers", requireAuth, requireRole("team_lead","division_manager","sales_director","admin"), async(req,res)=>{
  try {
    ...
    if (req.user.role === "sales_director" || req.user.role === "admin") {
      // Sales director / admin mund tÃ« shohÃ« tÃ« gjithÃ« aprovuesit
      r = await q(
      ...
      // Team lead / division manager sheh vetÃ«m aprovuesit e divisionit tÃ« vet
      r = await q(
        `SELECT id,first_name,last_name,email,role,division_id FROM users
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ DELEGATIONS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
app.post("/delegations", requireAuth, requireRole("team_lead","division_manager","sales_director"), async(req,res)=>{
  ...
  if(!to_user_id||!start_date||!end_date)return res.status(400).json({error:"Parametra mungojnÃ«"});
  if(Number(to_user_id)===req.user.id)return res.status(400).json({error:"Nuk mund tÃ« delegosh te vetja"});
  if(new Date(start_date)>new Date(end_date))return res.status(400).json({error:"Data e fillimit duhet tÃ« jetÃ« para datÃ«s sÃ« mbarimit"});
  try{
    const r=await q(
    ...
    if(delegate.rows[0]?.email){
      sendMail({
        ...
        html:`<p>PÃ«rshÃ«ndetje <b>${delegate.rows[0].first_name}</b>,</p><p>Aprovimi Ã«shtÃ« deleguar te ju nga <b>${req.user.first_name} ${req.user.last_name}</b> pÃ«r periudhÃ«n <b>${start_date} â€” ${end_date}</b>.</p><p>GjatÃ« kÃ«saj periudhe do t'u caktohen kÃ«rkesat automatikisht.</p><a href="${APP_URL}/approvals" style="background:#1e3a5f;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;display:inline-block">Hap Aprovime â†’</a>`
      }).catch(()=>{});
    }
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ AGENT LIMITS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
app.post("/admin/agent-limits", requireAuth, requireRole("admin"), async(req,res)=>{
  ...
  if(!user_id||!period||!max_amount)return res.status(400).json({error:"Parametra mungojnÃ«"});
...
/* â”€â”€â”€ Monthly report trigger â”€â”€â”€ */
app.post("/admin/reports/run", requireAuth, requireRole("admin","sales_director"), async(req,res)=>{
  ...
  res.json({ok:true,message:`Raporti pÃ«r ${period} Ã«shtÃ« duke u gjeneruar`});
});
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ ADMIN CRUD (unchanged) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
app.post("/admin/users",requireAuth,requireRole("admin"),async(req,res)=>{try{
  ...
  if(!email?.trim()||!password?.trim())return res.status(400).json({error:"Email/fjalÃ«kalimi mungon"});
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ DASHBOARD STATS (extended) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
app.get("/approvals/export-csv",requireAuth,requireRole("sales_director","division_manager","admin"),async(req,res)=>{
  try{
    ...
    const headers=["ID","Data","Statusi","Niveli","Shuma","Agjenti","PDA","BlerÃ«si Kodi","BlerÃ«si Emri","Objekti","Ref FaturÃ«","Arsyeja","Aprovuesi","Data Aprovimit","Veprimi","Koment"];
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ REQUESTS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
app.post("/requests",requireAuth,requireRole("agent","admin"),upload.array("photos",10),async(req,res)=>{
  try{
    ...
    const buyerIdClean=cleanId(buyer_id);if(!buyerIdClean)return res.status(400).json({error:"Zgjedh blerÃ«sin"});
    ...
    if(Array.isArray(items)&&items.length){
      // Upsert artikujt nga PricingBridge nÃ« tabelÃ«n lokale articles (ON CONFLICT DO UPDATE)
      // Kjo siguron qÃ« article_id FK tÃ« jetÃ« gjithmonÃ« valid
    ...
    try{
      ...
      for(const lim of limits.rows){
        ...
        if(Number(spent.rows[0].s)+totalAmount>Number(lim.max_amount))
          return res.status(400).json({error:`Tejkaluat limitin ${lim.period==="monthly"?"mujor":"javor"} (â‚¬${Number(lim.max_amount).toFixed(2)}). Keni shpenzuar â‚¬${Number(spent.rows[0].s).toFixed(2)} deri tani.`});
      }
...
/* â”€â”€â”€ Start â”€â”€â”€ */
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ ADMIN SYNC ENDPOINT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */

/* ─────────────────── OCR ─────────────────── */

  const base64Data = image.replace(/^data:image\/\w+;base64,/, "");
  const tmpId = crypto.randomBytes(8).toString("hex");
  const tmpInput = path.join("/tmp", `ocr-in-${tmpId}.jpg`);
  const tmpOutput = path.join("/tmp", `ocr-out-${tmpId}`);
  try {
    fs.writeFileSync(tmpInput, Buffer.from(base64Data, "base64"));
    await new Promise((resolve, reject) => {
      execFile("tesseract", [tmpInput, tmpOutput, "-l", "eng", "--psm", "6"], { timeout: 15000 }, (err, _, stderr) => {
        if (err) reject(new Error(stderr || err.message)); else resolve();
      });
    });
    const raw = fs.readFileSync(`${tmpOutput}.txt`, "utf8");
    const clean = raw.replace(/\s+/g, " ").trim().toUpperCase().replace(/[^A-Z0-9\-\/\s]/g, "");
    try { fs.unlinkSync(tmpInput); fs.unlinkSync(`${tmpOutput}.txt`); } catch {}
    return res.json({ text: clean, raw: raw.trim() });
  } catch (err) {
    try { fs.unlinkSync(tmpInput); } catch {}
    console.error("[OCR]", err.message);
    return res.status(500).json({ error: "OCR deshtoi", detail: err.message });
  }
});
...
app.listen(PORT,()=>{
  ...
  startCronJobs();
  // Sync automatik gjatÃ« startit (pas 3 sekondash pÃ«r tÃ« lÃ«nÃ« DB tÃ« jetÃ« gati)
  setTimeout(async () => {
